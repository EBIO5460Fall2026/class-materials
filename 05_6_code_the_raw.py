...
t_max = 100
# isize = 20   #initial array size for individuals, must be > N_0 below
# psize = 20   #initial array size for population info, > 1

# Initial biological state

# --individual scale (parameters to set)
N_0 = 10
# alive = init(np.full(N_0, True), isize, False)  # alive state
# b = init(np.full(N_0, 0.25), isize, 0)          # birth rate
# m = init(np.full(N_0, 0.2), isize, 0)           # death rate
...
# N = init(np.full(1, N_t), psize, -1)          #track N
# times = init(np.full(1, t), psize, np.nan)    #track time
...
chkpt_t = 1
#while t < t_max and N_t > 0 and irow < isize and prow < psize:
