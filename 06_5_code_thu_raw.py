...
# Simulation control
rng = np.random.default_rng(38408)
sims = 1000
t_max = 100
...
# Mean and standard deviation of realizations
...
def birth_death_ibm_heterogeneous(alive, b, m, b_mn, b_cv, t_max, rng):

    # Initial biological state

    t = 0.0      #needs to be float, e.g 0.0

    # Birth rate gamma distribution parameters
    b_shape = 1 / b_cv**2
    b_scale = b_mn * b_cv**2
    ...
    while t < t_max and N_t > 0:
        ...
        # which individual does the event happen to?
        if event_birth:
            p_ind = b[alive_list[:N_t]] / b_intensity
            j = rng.choice(N_t, p=p_ind)
            i = alive_list[j]
        else:
            j = rng.integers(N_t)
            i = alive_list[j]
        ...
        if event_birth:
            # insert new individual in arrays
            alive[irow] = 1
            b[irow] = rng.gamma(b_shape, b_scale)  #individual variation
            m[irow] = m[i]  #inherit from parent
            # update alive list
            alive_list[N_t] = irow
            # update intensities
            b_intensity = b_intensity + b[irow]   #also update here
            m_intensity = m_intensity + m[i]
            # update N
            N_t = N_t + 1
            # increment row index for next time
...
alive = np.full(N_0, True)  # alive state
b = rng.gamma(b_shape, b_scale, N_0) # birth rate
m = np.full(N_0, 0.2)       # death rate
...
for i in range(sims):
    N_series, t = birth_death_ibm_heterogeneous(alive, b, m, b_mn, b_cv, t_max, rng)
    result.append((N_series, t))
    if i % 100 == 0:
        print(i)
